    ...
    <v-col cols="8">
      <v-col class="form-header rounded-lg">
        <h1 class="font-weight-light">
          Applicants for <span class="font-weight-regular">{{ JobTitle }}</span> position
        </h1>
              ...
              <v-row>
                <v-col cols="8">
                  <h4 class="mb-2 ml-4 grey--text">
                    Skills:
                    <v-chip color="#01579B" text-color="white" class="mr-2" small v-for="skill in Skills" :key="skill.id">{{ skill.id }} </v-chip>
                  </h4>
                  <h4 class="mb-2 ml-4 grey--text">
                    Education: <span class="black--text">{{ Education }}</span>
                  </h4>
                  <h4 class="mb-2 ml-4 grey--text">
                    Location: <span class="black--text">{{ Location }}</span>
                  </h4>
                  <h4 class="mb-2 ml-4 grey--text">
                    Phone: <span class="black--text">{{ Phone }}</span>
                  </h4>
                  <h4 class="mb-2 ml-4 grey--text">
                  ...
                  <v-col cols="8">
                    ...
                    <v-btn class=" ml-3" :loading="loading2" :disabled="loading2" dark color="orange darken-2" @click="loader = 'loading2'">
                      Reject
                      <template v-slot:loader>
                        <span>Loading...</span>
                      </template>
                ...
                <v-col cols="4">
                  <vue-ellipse-progress :progress="requirmentsPercent" :size="150" :legend-value="requirments" >
...
export default {
  data() {
    return {
      ...
      requirmentsDone: 75,
      Skills: [{ id: ".NET" }, { id: "HTML" }, { id: "MySQL" }, { id: "Javascript" }],
      //action buttons
      loading: false,
      loading2: false,
      loader: null,
